...
def _run_migrations(engine) -> None:
    ...
    if insp.has_table("fund_allocation_overrides"):
        ...
        with engine.begin() as conn:
            ...
            if "mode" not in fao_cols:
                conn.execute(text(
                    "ALTER TABLE fund_allocation_overrides ADD COLUMN mode VARCHAR(20)"
                ))
    ...
    # B31: Rewrite FKs pointing at accounts.id to carry ON DELETE CASCADE / SET NULL.
    # Postgres-only: sqlite (dev) doesn't enforce FKs anyway (no PRAGMA), and
    # ALTER TABLE constraint edits are not supported on sqlite pre-3.35 without
    # a full table rebuild. For fresh sqlite installs, create_all() already emits
    # the new DDL from models.py.
    if is_pg:
        with engine.begin() as conn:
            already = conn.execute(
                text("SELECT 1 FROM _migrations WHERE name = 'account_fk_cascade_v1'")
            ).fetchone()
            if not already:
                # For each (table, column, target_ondelete) tuple: look up the
                # existing FK constraint name from information_schema, drop it,
                # then re-add with the correct ON DELETE clause. Names are auto-
                # generated by Postgres (e.g. "cards_account_id_fkey") but we
                # discover them dynamically rather than assume the naming.
                fk_targets = [
                    ("cards", "account_id", "CASCADE"),
                    ("payment_methods", "linked_account_id", "SET NULL"),
                    ("fund_accounts", "account_id", "CASCADE"),
                    ("postings", "account_id", "CASCADE"),
                    ("recurring_transactions", "account_id", "CASCADE"),
                    ("recurring_transactions", "from_account_id", "CASCADE"),
                    ("recurring_transactions", "to_account_id", "CASCADE"),
                ]
                for table, column, on_delete in fk_targets:
                    ...
                    conn.execute(text(
                        ...
                        f"FOREIGN KEY ({column}) REFERENCES accounts(id) "
                        f"ON DELETE {on_delete}"
                    ))
                conn.execute(text(
                    "INSERT INTO _migrations (name) VALUES ('account_fk_cascade_v1')"
                ))

    # B29: Rewrite FKs pointing at funds.id to carry ON DELETE CASCADE / SET NULL.
    # Same rationale as account_fk_cascade_v1 above (Postgres-only, sqlite gets
    # the new DDL via create_all() on fresh installs).
    if is_pg:
        with engine.begin() as conn:
            already = conn.execute(
                text("SELECT 1 FROM _migrations WHERE name = 'fund_fk_cascade_v1'")
            ).fetchone()
            if not already:
                fk_targets = [
                    ("fund_allocation_overrides", "fund_id", "CASCADE"),
                    ("fund_accounts", "fund_id", "CASCADE"),
                    ("transactions", "fund_id", "SET NULL"),
                    ("transactions", "source_fund_id", "SET NULL"),
                    ("transactions", "dest_fund_id", "SET NULL"),
                    ("recurring_transactions", "fund_id", "SET NULL"),
                    ("recurring_transactions", "source_fund_id", "SET NULL"),
                    ("recurring_transactions", "dest_fund_id", "SET NULL"),
                ]
                for table, column, on_delete in fk_targets:
                    rows = conn.execute(text("""
                        SELECT tc.constraint_name
                    ...
                    conn.execute(text(
                        f"ALTER TABLE {table} "
                        f"ADD CONSTRAINT {table}_{column}_fkey "
                        f"FOREIGN KEY ({column}) REFERENCES funds(id) "
                        f"ON DELETE {on_delete}"
                    ))
                conn.execute(text(
                    "INSERT INTO _migrations (name) VALUES ('fund_fk_cascade_v1')"
                ))

    # B30: Rewrite FKs pointing at categories.id and subcategories.id to carry
    # ON DELETE CASCADE / SET NULL. Same rationale and pattern as
    # account_fk_cascade_v1 / fund_fk_cascade_v1 above (Postgres-only; sqlite
    # gets the new DDL via create_all() on fresh installs and does not enforce
    # FKs at runtime without PRAGMA foreign_keys=ON).
    if is_pg:
        with engine.begin() as conn:
            already = conn.execute(
                text("SELECT 1 FROM _migrations WHERE name = 'category_fk_cascade_v1'")
            ).fetchone()
            if not already:
                # (table, column, parent_table, on_delete)
                fk_targets = [
                    ("subcategories", "category_id", "categories", "CASCADE"),
                    ("transactions", "category_id", "categories", "SET NULL"),
                    ("transactions", "subcategory_id", "subcategories", "SET NULL"),
                    ("recurring_transactions", "category_id", "categories", "SET NULL"),
                    ("recurring_transactions", "subcategory_id", "subcategories", "SET NULL"),
                    ("recurring_transactions", "fee_category_id", "categories", "SET NULL"),
                ]
                for table, column, parent, on_delete in fk_targets:
                    rows = conn.execute(text("""
                        SELECT tc.constraint_name
                        FROM information_schema.table_constraints tc
                        JOIN information_schema.key_column_usage kcu
                ...
                conn.execute(text(
                    "INSERT INTO _migrations (name) VALUES ('category_fk_cascade_v1')"
                ))
